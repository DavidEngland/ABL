...
## Optional Challenge 2: Numerical Integration Accuracy (+5%)

Implement trapezoid and Simpson's rules to compute $Ri_b$ from a discretized $Ri_g(z)$ profile.

**Given:**  
$Ri_g(z) = c_1 z + c_2 z^2$ (quadratic), $z_0 = 10$ m, $z_1 = 100$ m, $c_1 = 0.01$, $c_2 = -0.0001$.

**Tasks:**
1. Evaluate $Ri_g$ at $z_0, z_g, z_1$ where $z_g = \sqrt{z_0 z_1}$.
2. Compute $Ri_b^{\text{trap}}$ using trapezoid rule: $\frac{1}{2}[Ri_g(z_0) + Ri_g(z_1)]$.
3. Compute $Ri_b^{\text{Simpson}}$ using Simpson's rule: $\frac{1}{6}[Ri_g(z_0) + 4Ri_g(z_g) + Ri_g(z_1)]$.
4. Compare with analytic $Ri_b = \frac{1}{\Delta z}\int_{z_0}^{z_1} Ri_g(z)\,dz$ (exact for polynomial).
5. Compute relative errors: $|\frac{Ri_b^{\text{method}} - Ri_b^{\text{exact}}}{Ri_b^{\text{exact}}}|$.

**Expected result:** Simpson more accurate than trapezoid for concave-down quadratic.
