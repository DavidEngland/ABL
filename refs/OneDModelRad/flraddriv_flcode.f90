...
program main
        ...
        real,dimension(100):: p,t,rho,rv,cw,ci,rr,gr,o3r,fthrd,htr
        real cosz, solconst, alb, em, sft, rshort, rlong
        integer nz

        nz = 63
        open(unit=10,file='inp.dat',status='old')
        read(10,*)cosz,alb,sft,solconst
        do i= 1, 63
           read(10,*)p(i),t(i),rho(i),rv(i),o3r(i),htr(i)
           cw(i)=0.0
           ci(i) =0.0
           rr(i) = 0.0
           gr(i) =0.0
        enddo
        em = 1.0
        call FuLiou_Raddriv (p,t,rho,rv,cw,ci,rr,gr,&
                             o3r, nz, cosz, solconst,&
                             alb,em,sft,&
                             rshort, rlong, fthrd)
        print*,'   Shortwave down at surface =',rshort
        print*,'   Longwave  down at surface =',rlong
        print*,'   _______________________________'
        print*,'   Level(mb)    Heating rate (K/s)'
        print*,'   _______________________________'
        do i= 1, 63
           print*,p(i)/100.0, fthrd(i)
        enddo
end
