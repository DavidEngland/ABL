...
!-------------------------------------------------------------------------------
	...
	Integer IRadswitch,Icountrad,IRadcall
	REAL FLUXIN,cp,rhoa,DELTRad
!-------------------------
!Set atmospheric radiation on IRadswitch =1
!			   off IRadswitch=0
        If ( Delt.GT.5)  IRadcall=1
	If ( Delt.LE.5)  IRadcall=2
	If ( Delt.LE.2)  IRadcall=3
	If ( Delt.LE.1) IRadcall=6
	If ( Delt.LE.0.5) IRadcall=10
	If ( Delt.LE.0.1) IRadcall=50
...
!-------------------------------------------------------------------------------
	...
	ALLOCATE(OBS_INSOL(TSTEPS))
        ...
        CALL EKMAN(UG, VG, Z, KZ, U, V, KM, KH, LAT)
...
!----------------------------
	...
	Cosz=-3.
	If (Iradswitch.EQ.1) then
	IcountRad=IcountRad+1
	IF (ICountRad.GE.IRADCALL) then
	DeltRad=IRADCALL*Delt
        IcountRad=0
	
	Call RADIATION (DeltRad,P,T,Q,RHOP,ozone,cosz, &
...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE EKMAN(UG, VG, Z, KZ, U, V, KM, KH, PHI)
   !include 'BL.Dat'
...
!-------------------------------------------------------------------------------
	...
	REAL    ::      TAERO, U, V, Z, Z0,T1, USTAR, TSTAR, L, VONKAR
...
!-------------------------------------------------------------------------------
	...
	REAL    ::      L, USTAR, TSTAR, Z, T1,Z0, TOLL, OLDL
...
!-------------------------------------------------------------------------------
	...
	REAL	::	L, TSTAR, Z, T, T0, Z0, VONKAR, TAERO
	REAL	::	PHIH, SIH, DENOM, ZOL, stability
	REAL	::	LOGZOZ0
...
!--------------------------------------------------------------------------------
	...
	REAL	::	USTAR, TSTAR,T1, L, Z, TAERO, VONKAR, G
...
!--------------------------------------------------------------------------------
	...
	REAL            :: LAT, T0, T, USTAR, TSTAR, DELT, ZI
